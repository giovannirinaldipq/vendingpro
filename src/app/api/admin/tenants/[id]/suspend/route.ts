...
import { sendTenantSuspended } from '@/lib/email/send';
...
export async function POST(
  ...
  if (prev.subscription_status === 'suspended') {
    return NextResponse.json({ success: false, error: { code: 'INVALID_STATE', message: 'Cliente já está suspenso' } }, { status: 409 });
  }
  ...
  await logAudit({
    ...
    newValues: { subscription_status: 'suspended', reason: parsed.data.reason },
    ipAddress: meta.ipAddress,
    userAgent: meta.userAgent,
  });

  if (parsed.data.notify_email && prev.contact_email) {
    void sendTenantSuspended(
