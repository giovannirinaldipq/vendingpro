  ...
  useEffect(() => {
    (async () => {
      ...
      if (process.env.NODE_ENV === 'development') {
        const testPredictions: Prediction[] = [
          {
            ...
            days_of_stock: 0,
            estimated_runout_date: new Date(Date.now() + 1 * 24 * 60 * 60 * 1000).toISOString(),
            suggested_purchase_quantity: 24,
            status: 'critical',
            machine_id: 'machine-1',
            machine_name: 'Máquina 1 - Shopping',
            trend: 'increasing',
            popularity_rank: 1,
          },
          {
            product_id: '2',
            product_name: 'Guarantá Lata 500ml',
            current_quantity: 15,
            avg_daily_consumption: 6,
            days_of_stock: 2,
            estimated_runout_date: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString(),
            suggested_purchase_quantity: 18,
            status: 'warning',
            machine_id: 'machine-1',
            machine_name: 'Máquina 1 - Shopping',
            trend: 'stable',
        ...
        const testSchedule: Schedule[] = [
          {
            machine_id: 'machine-1',
            machine_name: 'Máquina 1 - Shopping',
            next_visit_date: new Date(Date.now() + 2 * 24 * 60 * 60 * 1000).toISOString(),
            days_since_last_visit: 1,
            avg_daily_sales: 45,
            low_stock_items: 2,
            suggested_visit_date: new Date(Date.now() + 2 * 24 * 60 * 60 * 1000).toISOString(),
            urgency: 'medium',
          },
        ];

        const testPurchase: PurchaseItem[] = [
          {
            product_id: '1',
            product_name: 'Coca-Cola Lata 350ml',
            current_quantity: 5,
            avg_daily_consumption: 8,
            suggested_purchase_quantity: 24,
            suggested_purchase_date: new Date(Date.now() + 1 * 24 * 60 * 60 * 1000).toISOString(),
          },
        ];

        const testSwaps: SwapSuggestion[] = [
          {
            product_id: 'old',
            product_name: 'Refrigerante A',
            current_quantity: 20,
            avg_daily_consumption: 2,
            popularity_rank: 8,
            suggested_replacement_product_id: 'new',
            suggested_replacement_product_name: 'Refrigerante B',
            suggested_replacement_reason: 'Maior margem de lucro e consumo similar',
            potential_impact: {
        ...
        setPurchase(testPurchase);
        setSwaps(testSwaps);
        setCapacity({ active_restockers_count: 2, avg_visits_per_day_per_restocker: 3 });
        setPicklist([]);
        setLoading(false);
        return;
      }
